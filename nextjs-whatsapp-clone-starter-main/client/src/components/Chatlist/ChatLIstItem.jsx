...
function ChatLIstItem({data,isContactsPage =false}) {
  ...
  const handleContactClick = () => {
    ...
    if(!isContactsPage){
      dispatch({type:reduceCases.CHANGE_CURRENT_CHAT_USER, 
        user:{
          name:data.name,
          about:data.about,
      ...
      <div className="flex  border-b border-conversation-border pb-2 pt-1 p3-2">
        <div className="flex justify-between w-full">
          <span className="text-secondary line-clamp-1 text-sm">{ isContactsPage ?data?.about || "\u00A0" : 
            ...
            {
              data.totalUnreadMessages >0 && <span className="bg-icon-green px-[6.4px] rounded-full text-sm">{data.totalUnreadMessages}</span>
            }
